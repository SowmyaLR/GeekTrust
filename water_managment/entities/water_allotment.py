...
class WaterAllotment:
    ...
    def calculate_water_consumption(self, max_persons):
        apt_water_consumption = max_persons * WATER_ALLOTED_FOR_INDIVIDUAL * TOTAL_DAYS
        self.total_cost += math.ceil(apt_water_consumption * self.corp * CORP_WATER) / (self.corp + self.bore) + \
                           math.ceil(apt_water_consumption * self.bore * BORE_WATER) / (self.corp + self.bore)
        self.total_water += apt_water_consumption
    ...
    def calculate_tanker_water_consumption(self, guest_size, no_of_days, available_accomodation):
        if available_accomodation <= 0:
            total_water_req = guest_size * WATER_ALLOTED_FOR_INDIVIDUAL * no_of_days
